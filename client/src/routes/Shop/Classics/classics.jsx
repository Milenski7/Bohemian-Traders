...
const Classics = ({ history }) => {
    ...
    const getProducts = async () => {
        try {
            const { data } = await http.get(`${config.apiEndpoint}/product/Classics`);
            setProducts(data);
    ...
    return (
        <React.Fragment>
            <HistoryLine active="shop/classics" />
                        ...
                        {products && newSortedProducts.map(product => (
                            <div key={product._id} className="products-grid-item">
                                <img
                                    ...
                                    onClick={() => history.push(`/shop/classics/${product._id}`)} />
                                <p className="products-grid-title">{product.title}</p>
                                <p className="products-grid-price">{product.price.toFixed(2)}$</p>
                                <button onClick={() => history.push(`/shop/classics/${product._id}`)} className="products-grid-button">VIEW PRODUCT</button>
                            </div>
                        ))}
                    </div>}
                </div>
