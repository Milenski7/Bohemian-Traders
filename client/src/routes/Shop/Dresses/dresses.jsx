...
const Dresses = ({ history }) => {
    ...
    const getProducts = async () => {
        try {
            const { data } = await http.get(`${config.apiEndpoint}/product/Dresses`);
            setProducts(data);
    ...
    return (
        <React.Fragment>
            <HistoryLine active="shop/dresses" />
            ...
            <div className="products-container">
                <div className="products">
                    ...
                    <div className="categories-container">
                        ...
                        <p onClick={() => changeCategory('Maxi')} className="category">MAXI</p>
                        <p onClick={() => changeCategory('Midi')} className="category">MIDI</p>
                        <p onClick={() => changeCategory('Mini')} className="category">MINI</p>
                    </div>
                        ...
                        {products && newSortedProducts.map(product => (
                            <div key={product._id} className="products-grid-item">
                                <img
                                    ...
                                    onClick={() => history.push(`/shop/dresses/${product._id}`)} />
                                <p className="products-grid-title">{product.title}</p>
                                <p className="products-grid-price">{product.price.toFixed(2)}$</p>
                                <button onClick={() => history.push(`/shop/dresses/${product._id}`)} className="products-grid-button">VIEW PRODUCT</button>
                            </div>
                        ))}
                    </div>}
                </div>
